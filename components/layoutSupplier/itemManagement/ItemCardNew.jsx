...
export default function ItemCardNew() {
  ...
  const handleSave = async () => {
    console.log(data)
    if (!data.name || !data.product_type || !data.price) return;

    setSaving(true);

    const newProduct = await query("addNewProduct", {
      sup: user,
      product: {
        ...data,
        from_supplier: user.name,
        price_offer:
    ...
    setAll((prevAll) => [...prevAll, {...newProduct.product}]);
        ...
        <ScrollView className={`relative mb-[130px]`}>
          ...
          >
            ...
            <TouchableOpacity>
              <Image
                source={{
                  uri: `${API_SERVER}/img/products/stock.png`,
                }}
            ...
            <TouchableOpacity onPress={handleSave} className={`my-12`}>
              <Text
                className={`rounded-3xl border-transparent bg-green-500 px-16 py-2 text-center font-[400] tracking-wider text-white shadow-lg shadow-black`}
              >
                Save
              </Text>
            </TouchableOpacity>
